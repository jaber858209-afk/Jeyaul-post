            ...
            >
              ...
              <div className="bg-white/5 border border-white/10 rounded-3xl p-8 flex flex-col h-full">
                <div className="flex items-center justify-between mb-6">
                  ...
                  >
                    {copied === 'li' ? <Check size={18} className="text-green-400" /> : <Copy size={18} />}
                  </button>
                </div>
                <p className="text-white/80 leading-relaxed mb-6 whitespace-pre-wrap flex-grow">
                  {result.linkedin.text}
                </p>
                <div className="flex flex-wrap gap-2 mb-6">
                  {result.linkedin.hashtags.map((tag, i) => (
                    <span key={i} className="text-sm text-blue-400 font-medium">#{tag}</span>
                  ))}
                </div>
                {image && (
              ...
              <div className="bg-white/5 border border-white/10 rounded-3xl p-8 flex flex-col h-full">
                ...
                <div className="text-[10px] font-mono text-white/20 uppercase tracking-widest mb-4">
                  Characters: {result.twitter.text.length + result.twitter.hashtags.join(' ').length + 2} / 280
                </div>
                {image && (
                  <div className="relative aspect-video rounded-xl overflow-hidden border border-white/10 group">
                    <img src={image} alt="Generated" className="w-full h-full object-cover" referrerPolicy="no-referrer" />
